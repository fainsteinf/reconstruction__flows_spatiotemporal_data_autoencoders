...
#Code for: train autoencoders with a 3-d latent space
...
#number of pixels that each image has
...
#Every saving_step epochs a checkpoint of the trained model will be saved 
saving_step = 10
#%%
#Measure time
start = time.time()
...
for ntrain in range(N_trains):
    ...
    encoded = layers.Dense(16, activation='relu', 
                           kernel_initializer=initializers.RandomNormal(mean=mean_init, 
                                                                        stddev=stdde_init))(encoded)
    encoded = layers.Dense(3, activation='linear', 
                           kernel_initializer=initializers.RandomNormal(mean=mean_init, 
                                                                        stddev=stdde_init))(encoded)
    ...
    filepath=saving_dir+"{}".format(ntrain)+"-checkpoint-epoch-{epoch:03d}.hdf5"
    filepath_best=saving_dir+"{}-checkpoint-best.hdf5".format(ntrain)
    checkpoint_epoch = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_freq='epoch', period=saving_step)
    checkpoint_best = ModelCheckpoint(filepath_best, monitor='val_loss', verbose=1, save_best_only=True, mode='min')
    callbacks_list = [checkpoint_epoch, checkpoint_best]
